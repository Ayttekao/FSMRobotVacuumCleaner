...
public class LeeAlgorithm
{
    ...
    public void SetStarCell(int startX, int startY)
    {
        ...
        if (startY > ArrayGraph.GetLength(1) || startY < 0)
        {
    ...
    public void SetDestinationCell(int startX, int startY)
    {
        if (startX > ArrayGraph.GetLength(0) || startX < 0)
        {
            throw new ArgumentException("Неправильная координата x");
        ...
        ArrayGraph[startX, startY] = (int)Figures.Destination;
    }
            ...
            for (var i = 0; i < w; i++)
            {
                for (var j = 0; j < h; j++)
                {
                    if (ArrayGraph[i, j] == _step)
                    {
                        ...
                        if (i != w - 1)
                        {
                            if (ArrayGraph[i + 1, j] == (int)Figures.EmptySpace)
                            {
                                ArrayGraph[i + 1, j] = _step + 1;
                            }
                        }
                        if (j != h - 1)
                        {
                            if (ArrayGraph[i, j + 1] == (int)Figures.EmptySpace)
                            {
                                ArrayGraph[i, j + 1] = _step + 1;
                            }
                        }
                        if (i != 0)
                        {
